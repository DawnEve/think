...
class SeqModel extends Model {
	...
	function getData($by,$id,$uid=0){
        ...
        if($by=='' or $id==0){
            $info = $this
                ->where('`condition`>0 and seq_uid='.$uid)  
                ->select();
            return $this->id2name($info);
        }elseif($by=='cate'){ //debug($id);
            $info = $this
                ->where('`condition`>0 and seq_uid='.$uid.' and cate_id='.$id)  
                ->select();
            ...
            $info = $this
                ->where('`condition`>0 and seq_uid='.$uid.' and tag_ids REGEXP "(^|,)'.$id.'($|,)"')  
                ->select();
                //tag_ids REGEXP '(^|,)5($|,)';
               ...
               $current_tag_id_list=explode(',',$v['tag_ids']); //dump($current_tag_id_list);die();
                   foreach($current_tag_id_list as $current_tag_id){
                       if(array_key_exists($current_tag_id,$tag_list)){
                           ...
                           $info[$k]['tag_name_links'] .= "<a class=tag href='/Admin/Seq/showlist/by/tag/id/".$current_tag_id."'>".$current_tag_name."</a>";
